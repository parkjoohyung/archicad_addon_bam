...
// Define body index types for morph generation
...
namespace AddOnLogic {
    ...
    // --- Create Zone/Mesh/Morph from Selection ---
        ...
        if (polySources.empty()) {
            ...
            while (allLines.size() >= 3) {
                ...
                if (closed && pts.size() >= 3) {
                    ...
                    ps.originalType = API_ZombieElemID;
                    ps.nCoords = (Int32)pts.size(); ps.nSubPolys = 1; ps.nArcs = 0;
                    ps.memo.coords = (API_Coord**)BMAllocateHandle((ps.nCoords+1)*sizeof(API_Coord), ALLOCATE_CLEAR, 0);
                    ps.memo.pends = (Int32**)BMAllocateHandle(2*sizeof(Int32), ALLOCATE_CLEAR, 0);
                    if (ps.memo.coords && ps.memo.pends) {
        ...
        for (auto& ps : polySources) {
            ACAPI_CallUndoableCommand("Create Element", [&]() -> GSErrCode {
                ...
                if (el.header.type == API_MorphID) {
                    ...
                    memo.coords = ps.memo.coords; memo.pends = ps.memo.pends; memo.parcs = ps.memo.parcs;
                    if (isMesh) {
            ...
            while (std::getline(file, line)) {
                ...
                if (ACAPI_Element_GetDefaults(&zoneElem, nullptr) == NoError) {
                    ...
                    if (memo.coords && memo.pends) {
                        ...
                        if (ACAPI_Element_Create(&zoneElem, &memo) == NoError) createdCount++;
                    }
                    ACAPI_DisposeElemMemoHdls(&memo);
                }
                storyCurX[storyIdx] = curX + side + 1.0;
            }
            ACAPI_WriteReport(GS::UniString::Printf("Created %d Zones from Excel.", createdCount), false);
            return NoError;
        });
    ...
    // --- Reshape Elements to Target Area ---
    ...
    void Resize3D(double scale, Int32 targetW, Int32 targetH, Int32& inOutOrigWidth, Int32& inOutOrigHeight) {
        ...
        if (ACAPI_View_Get3DWindowSets(&windowInfo) == NoError) {
            if (inOutOrigWidth == 0 || inOutOrigHeight == 0) {
                inOutOrigWidth = windowInfo.hSize;
                inOutOrigHeight = windowInfo.vSize;
            }
    ...
    // --- Match Aspect Ratio with Selected Drawing ---
    ...
    // --- HeightPalette: MeshElevator and GL Calculation ---
    class MeshElevator {
    public:
        struct Triangle {
            API_Coord v1, v2, v3;
    ...
    bool CalculateGL(const API_Guid& morphGuid, const API_Guid& meshGuid, GLResult& result, std::vector<API_Coord>& outFootprint, short& outFloorInd) {
        API_Element moE; BNZeroMemory(&moE, sizeof(moE));
        moE.header.guid = morphGuid;
        if (ACAPI_Element_Get(&moE) != NoError) return false;
        outFloorInd = moE.header.floorInd;

        API_ElementMemo memo; BNZeroMemory(&memo, sizeof(memo));
        if (ACAPI_Element_GetMemo(morphGuid, &memo) != NoError) return false;

        std::vector<API_Coord> fp;
        if (memo.coords) {
            Int32 nC = BMGetHandleSize((GSHandle)memo.coords) / sizeof(API_Coord);
            for (Int32 i = 1; i < nC; ++i) {
                if ((*memo.coords)[i].x != 0 || (*memo.coords)[i].y != 0) {
        ...
        double moBZ = moE.morph.level, sumRZ = 0, sBZ = 1e30, maxRelZ = 0;
        std::vector<double> vZs;
        double step = 0.05;

        MeshElevator elevator(meshGuid);
        if (elevator.GetTriangleCount() == 0) return false;

        for (double x = minX; x <= maxX; x += step) {
            for (double y = minY; y <= maxY; y += step) {
                API_Coord p = { x, y };
                if (IsPointInPolygon(p, fp)) {
        ...
        if (!vZs.empty()) {
            for (double z : vZs) {
                double r = z - sBZ;
                if (r > maxRelZ) maxRelZ = r;
                sumRZ += std::min(3.0, std::max(0.0, r));
            }
            double aR = sumRZ / (double)vZs.size();
            result.baseZ = sBZ;
            result.maxRelZ = maxRelZ;
            result.finalZ = sBZ + aR;
            result.contactArea = (double)vZs.size() * step * step;
            result.ptCount = (int)vZs.size();
            return true;
        }
    ...
    void CreateGLGround(const std::vector<API_Coord>& footprint, double finalZ, short floorInd) {
        ...
        auto op = OffsetPolygon(footprint, 1.0);
        API_Element el; BNZeroMemory(&el, sizeof(el)); el.header.type = API_MorphID; ACAPI_Element_GetDefaults(&el, nullptr);
        el.header.floorInd = floorInd; el.morph.level = 0.0;
        
        void* bD = nullptr; ACAPI_Body_Create(nullptr, nullptr, &bD);
        std::vector<VertIdx> vIs;
        for (auto& p : op) {
            VertIdx vI; API_Coord3D c; c.x = p.x; c.y = p.y; c.z = finalZ;
            ACAPI_Body_AddVertex(bD, c, vI);
            vIs.push_back(vI);
        }
        if (vIs.size() >= 3) {
            PolyIdx pI;
        ...
        if (ACAPI_Body_Finish(bD, &memo.morphBody, &memo.morphMaterialMapTable) == NoError) {
            ACAPI_CallUndoableCommand("Create GL", [&]() {
                return ACAPI_Element_Create(&el, &memo);
            });
            ACAPI_DisposeElemMemoHdls(&memo);
        }
    }

    // --- TerrainPalette Contours Generator ---
    void GenerateContours(double divisionsVal, short penIndex) {
        ...
        if (hasMesh) {
            // DrawContoursFromMesh
            MeshElevator elevator(meshGuid);
            if (elevator.triangles.IsEmpty()) return;

            double step = divisionsVal;
            ...
            struct LineSegment { API_Coord p1, p2; };
                ...
                for (double H : levels) {
                    ...
                    for (const auto& tri : elevator.triangles) {
                        ...
                        if (intersectPts.size() >= 2) {
                            LineSegment seg; seg.p1 = intersectPts[0]; seg.p2 = intersectPts[1];
                    ...
                    for (size_t i = 0; i < segments.size(); ++i) {
                        ...
                        while (extended) {
                            ...
                            for (size_t j = 0; j < segments.size(); ++j) {
                                ...
                                if (d_tail_p1 < tolerance) { currentPoly.push_back(segments[j].p2); used[j] = true; extended = true; break; }
                                else if (d_tail_p2 < tolerance) { currentPoly.push_back(segments[j].p1); used[j] = true; extended = true; break; }
                                else if (d_head_p1 < tolerance) { currentPoly.insert(currentPoly.begin(), segments[j].p2); used[j] = true; extended = true; break; }
                                else if (d_head_p2 < tolerance) { currentPoly.insert(currentPoly.begin(), segments[j].p1); used[j] = true; extended = true; break; }
                            }
                        }
                    ...
                    for (const auto& polyCoords : polylines) {
                        API_Element element; BNZeroMemory(&element, sizeof(element));
                        element.header.type = API_PolyLineID;
                        ACAPI_Element_GetDefaults(&element, nullptr);
                        element.polyLine.linePen.penIndex = penIndex;
                        ...
                        if (memo.coords) {
                            ...
                            for (Int32 j = 0; j < nCoords; ++j) (*memo.coords)[j + 1] = polyCoords[j];
                        }
                        memo.pends = (Int32**)BMAllocateHandle(2 * sizeof(Int32), ALLOCATE_CLEAR, 0);
                        if (memo.pends) { (*memo.pends)[0] = 0; (*memo.pends)[1] = nCoords; }

                        GSErrCode err = ACAPI_Element_Create(&element, &memo);
                        ACAPI_DisposeElemMemoHdls(&memo);
                        if (err == NoError) createdContours++;
                    }
                }
                return NoError;
            });
                ...
                if (ACAPI_Element_GetHeader(&h) == NoError) {
                    ...
                    if (h.type.typeID == API_PolyLineID || h.type.typeID == API_SplineID) {
                        ...
                        if (ACAPI_Element_GetMemo(h.guid, &memo) == NoError) {
                            if (memo.coords) {
                                Int32 nC = BMGetHandleSize((GSHandle)memo.coords) / sizeof(API_Coord);
                                for (Int32 i = 1; i < nC; ++i) fp.push_back((*memo.coords)[i]);
                            }
                            ACAPI_DisposeElemMemoHdls(&memo);
                        }
                    } else if (h.type.typeID == API_LineID) {
                        API_Element lineEl; lineEl.header = h;
                        if (ACAPI_Element_Get(&lineEl) == NoError) {
                            fp.push_back(lineEl.line.begC); fp.push_back(lineEl.line.endC);
                        }
                    }
                    if (fp.size() >= 2) {
                        PolylineData pd; pd.guid = h.guid; pd.coords = fp;
            ...
            for (size_t i = 0; i < polylines.size(); ++i) {
                for (size_t j = i + 1; j < polylines.size(); ++j) {
                    ...
                    if (distSq > maxDistSq) { maxDistSq = distSq; idxStart = i; }
                }
            }
            ...
            int divisions = (int)divisionsVal;
            ...
            auto RamerDouglasPeucker = [](const std::vector<API_Coord>& points, double epsilon) {
                ...
                auto pointLineDist = [](const API_Coord& pt, const API_Coord& lineStart, const API_Coord& lineEnd) {
                    ...
                    if (dx == 0 && dy == 0) return std::sqrt(std::pow(pt.x - lineStart.x, 2) + std::pow(pt.y - lineStart.y, 2));
                    ...
                    return std::sqrt(std::pow(pt.x - (lineStart.x + t * dx), 2) + std::pow(pt.y - (lineStart.y + t * dy), 2));
                };

                struct Frame { int start; int end; };
                std::vector<Frame> stack = { {0, (int)points.size() - 1} };
                std::vector<bool> keep(points.size(), false);
                keep[0] = true; keep[points.size() - 1] = true;

                while (!stack.empty()) {
                    Frame frame = stack.back(); stack.pop_back();
                    double dmax = 0; int index = frame.start;
                ...
                for (size_t i = 0; i < points.size(); ++i) if (keep[i]) res.push_back(points[i]);
                return res;
            };

            auto ResamplePolyline = [](const std::vector<API_Coord>& poly, double stepLen) {
                if (poly.size() < 2) return poly;
                std::vector<API_Coord> resampled; resampled.push_back(poly.front());
                double accumulated = 0.0;
                for (size_t i = 1; i < poly.size(); ++i) {
                    double dx = poly[i].x - poly[i-1].x; double dy = poly[i].y - poly[i-1].y;
                    double segLen = std::hypot(dx, dy); if (segLen < 1e-9) continue;
                    double remaining = segLen; API_Coord cur = poly[i-1];
                    while (accumulated + remaining >= stepLen) {
                        double needed = stepLen - accumulated; double t = needed / remaining;
                        cur.x = cur.x + t * (poly[i].x - cur.x); cur.y = cur.y + t * (poly[i].y - cur.y);
                        resampled.push_back(cur);
                        remaining = std::hypot(poly[i].x - cur.x, poly[i].y - cur.y); accumulated = 0.0;
                    }
                    accumulated += remaining;
                }
                if (std::hypot(poly.back().x - resampled.back().x, poly.back().y - resampled.back().y) > 1e-4)
            ...
            ACAPI_CallUndoableCommand("Interpolate 2D Contours", [&]() -> GSErrCode {
                for (size_t i = 0; i < polylines.size() - 1; ++i) {
                    ...
                    if (d0K_sq < d00_sq) std::reverse(poly2.begin(), poly2.end());
                    ...
                    auto getDist = [](const API_Coord& p1, const API_Coord& p2) { return std::hypot(p1.x - p2.x, p1.y - p2.y); };

                    cost[0][0] = getDist(A[0], B[0]);
                    for (int r = 1; r < N; ++r) cost[r][0] = cost[r-1][0] + getDist(A[r], B[0]);
                    for (int c = 1; c < M; ++c) cost[0][c] = cost[0][c-1] + getDist(A[0], B[c]);
                    ...
                    int r = N - 1; int c = M - 1; path.push_back({r, c});
                    while (r > 0 || c > 0) {
                        if (r == 0) c--;
                        else if (c == 0) r--;
                        else {
                            double c_diag = cost[r-1][c-1]; double c_left = cost[r][c-1]; double c_up = cost[r-1][c];
                            if (c_diag <= c_left && c_diag <= c_up) { r--; c--; }
                            else if (c_left < c_diag && c_left <= c_up) c--;
                            else r--;
                        }
                        path.push_back({r, c});
                    }
                    ...
                    for (int step = 1; step < divisions; ++step) {
                        ...
                        for (const auto& p : path) {
                            ...
                            if (interpLine.empty()) interpLine.push_back(pt);
                            else {
                        ...
                        if (interpLine.size() > 2) interpLine = RamerDouglasPeucker(interpLine, 0.01);
                        if (interpLine.size() >= 2) {
                            ...
                            if (memo.coords) {
                                (*memo.coords)[0].x = 0.0; (*memo.coords)[0].y = 0.0;
                                for (size_t j = 0; j < interpLine.size(); ++j) (*memo.coords)[j + 1] = interpLine[j];
                            }
                            memo.pends = (Int32**)BMAllocateHandle(2 * sizeof(Int32), ALLOCATE_CLEAR, 0);
                            if (memo.pends) { (*memo.pends)[0] = 0; (*memo.pends)[1] = (Int32)interpLine.size(); }

                            GSErrCode err = ACAPI_Element_Create(&element, &memo);
                            ACAPI_DisposeElemMemoHdls(&memo);
                            if (err == NoError) createdCount++;
                        }
                    }
                }
                return NoError;
